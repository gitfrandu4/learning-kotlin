...
//        println(i)
    ...
    for ((index, _) in animals.withIndex()) {
        println("${animals[index]} tiene ${animals[index].length} letras y está en la posición $index")
    }
...
fun mainControlExercises() {
    ...
    println(ex54(1, 99))
    println(ex56("Caracol", "Saltamontes"))
    println(ex56("Ana", "Sol"))
    println(ex56("b", "ytyutru"))
}
...
fun ex56(word1: String, word2: String): Boolean{
    for(c1 in word1) for(c2 in word2) if(c1 == c2) return true
    return false
}